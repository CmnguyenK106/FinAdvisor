...
func (s *PostgresStorage) SaveAgentRun(ctx context.Context, run AgentRun) error {
	...
	_, err = s.pool.Exec(ctx, `
		INSERT INTO agent_runs (run_id, status, answer, sources, warnings, created_at, updated_at)
	...
	`, run.RunID, run.Status, run.Answer, sourcesJSON, warningsJSON, run.CreatedAt, run.UpdatedAt)
	return err
}
...
func (s *PostgresStorage) GetAgentRun(ctx context.Context, runID string) (AgentRun, bool, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT run_id, status, answer, sources, warnings, created_at, updated_at
		FROM agent_runs
		WHERE run_id = $1;
	`, runID).Scan(&run.RunID, &run.Status, &run.Answer, &sourcesJSON, &warningsJSON, &run.CreatedAt, &run.UpdatedAt)
